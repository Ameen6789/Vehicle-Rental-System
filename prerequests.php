...
// echo "$vid";
        ...
        $result=mysqli_query($dbcon,"SELECT * from prerequests where username='$vid' ");
...
$vid="";
        ...
        if(isset($_POST['button2'])){
         ...
         $result2=mysqli_query($dbcon,"SELECT * from dupcar where slno='$vusername'");
         if(!$result2){
        // echo "hi";
                     die('could not connect to mysql server:');
           }
        ...
        //   echo "2";
            ...
            if($vname2==$vusername && $vstatus=='pending')
            {
                $query="INSERT INTO car(company,model,carnumber,price,seats,description) VALUES ('$vcompany','$vmodel','$vcarnumber','$vprice','$vseats','$vdescription')";
                // echo '<script> alert("Data Inserted Successfully")</script>';
              //  echo "$hello";
                if(mysqli_query($dbcon,$query)){
                $query="DELETE FROM request where slno='$vusername'";
                if(mysqli_query($dbcon,$query)){
                  $query="DELETE FROM dupcar where slno='$vusername'";
                  if(mysqli_query($dbcon,$query)){
                    $query="DELETE FROM prerequests where slno='$vusername'";
                    if(mysqli_query($dbcon,$query)){
